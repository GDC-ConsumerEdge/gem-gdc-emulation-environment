...
- `pre-commit run --all-files` — formatting, linting, license headers, and
  secret scanning.
...
- **Cluster Name Length & Kubernetes 63-char Hostname Limit**: GCE VM FQDNs take
  the form `<cluster_name>-<node>.<zone>.c.<project_id>.internal`. Kubernetes
  node registration fails if the FQDN exceeds **63 characters**
...
- **Secondary-network VNI derivation is list-order-dependent**:
  ...
  `vxlan_id + loop.index` — the VLAN ID never enters the calculation.
  Reordering, inserting, or removing an entry in `secondary_networks`
