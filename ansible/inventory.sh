...
cat <<EOF
{
  "all": {
    "vars": {
      "ansible_ssh_common_args": "-i ~/.ssh/google_compute_engine -o IdentitiesOnly=yes -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ControlMaster=auto -o ControlPersist=60m -o ServerAliveInterval=30 -o ServerAliveCountMax=6 -o ConnectionAttempts=100 -o ProxyCommand='gcloud compute start-iap-tunnel %h %p --listen-on-stdin --project=${GCP_PROJECT} --zone=${GEM_GCP_ZONE}'",
